...
(define leg-width 80)
(define leg-length 140)
(define leg-height 920)

(define (leg x y)
  (block leg-length leg-width leg-height  x y floor "yellow"))

(define cross-beam-length 40)
(define cross-beam-width matress-width)
(define cross-beam-height 80)

(define (cross-beam x y)
  (block cross-beam-length cross-beam-width cross-beam-height x y leg-height "blue"))


(define main-beam-length (- matress-length 80))
(define main-beam-width 40)
(define main-beam-height 80)
...
(define rear-mid-main-beam (main-beam (- centre-x
                                       (+ (/ matress-width 5.0)
                                          (/ main-beam-width 2.0)
                                          main-beam-width))
                                      cross-beam-length))
(define centre-main-beam (main-beam (- centre-x (/ main-beam-width 2.0)) cross-beam-length))
(define front-mid-main-beam (main-beam (+ centre-x 
                                          (/ matress-width 5.0)
                                          (/ main-beam-width 2.0))
                                      cross-beam-length))
